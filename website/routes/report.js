...
app.get("/:type", (req, res) => {
  ...
  if(!["bug", "suggestion", "user"].includes(type)) return res.redirect("/report")
...
app.post("/submitbug", async (req, res) => {
  ...
  if(content_type != "application/json") return res.status(400).send({ code: 400, message: "Invalid Content-Type Header" })
  var bug = req.body.bug;
  if(!bug) return res.status(400).send({ code: 400, message: "No Bug Provided" })
  ...
  return res.status(200).send({ code: 200, message: "Bug Reported Successfully" })
})

app.post("/sendsuggestion", async (req, res) => {
  var contentType = req.headers["content-type"];
  if(contentType != "application/json") return res.status(400).send({ code: 400, message: "Invalid Content-Type Header" })
  var object = {
    shopitem: "Shop Item",
    command: "Command",
    website: "Website",
  ...
  if(!Object.keys(object).includes(req.body.type)) return res.status(400).send({ code: 400, message: "Invalid Suggestion Type" })
  ...
  return res.status(200).send({ code: 200, message: "Suggestion Sent Successfully" })
})
...
app.post("/sendreport", async (req, res) => {
  ...
  if(!user) return res.status(400).send({ code: 400, message: "Unknown User" })
  ...
  return res.status(200).send({ code: 200, message: "Report Sent Successfully" })
})
