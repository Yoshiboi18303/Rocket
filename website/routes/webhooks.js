...
app.post("/infinity", async (req, res) => {
  ...
  if(secret != process.env.INFINITY_SECRET) return res.status(400).send({ code: 400, message: "Incorrect Secret" })
  var body = req.body;
  ...
  switch(body.type.toLowerCase()) {
    case "test":
      const test_webhook_embed = new MessageEmbed()
        ...
        .setDescription("A new test vote was recieved from IBL!\n\n**Vote for the bot [here](https://infinitybots.gg/bots/975450018360229908/vote)!**")
        .addField("Recieved At", `\`${utc(body.timeStamp).format("HH:MM:SS - MM/DD/YYYY")}\``)
    ...
    case "vote":
      const vote_embed = new MessageEmbed()
        ...
        .setTitle("New Vote!")
        .setDescription("A new vote was recieved from IBL!\n\n**Vote for the bot [here](https://infinitybots.gg/bots/975450018360229908/vote)!**")
        .addFields([
          {
            name: "Voted By",
            value: `\`${client.users.cache.get(body.userID).tag || body.userName}\``,
            inline: true
          },
          {
            name: "Recieved At",
  ...
  return res.status(200).send({ code: 200, message: "Vote Successfully Recieved" })
})

app.post("/void", async (req, res) => {
  var secret = req.headers.authorization;
  if(secret != process.env.VOID_SECRET) return res.status(400).send({ code: 400, message: "Incorrect Secret" })
  var body = req.body;
  switch(body.type.toLowerCase()) {
    case "test":
      const test_webhook_embed = new MessageEmbed()
        .setColor(colors.cyan)
        .setTitle("New Test Vote!")
        .setDescription("A new test vote was recieved from Void Bots!\n\n**Vote for the bot [here](https://infinitybots.gg/bots/975450018360229908/vote)!**")
        .addField("Recieved At", `\`${utc(body.timeStamp).format("HH:MM:SS - MM/DD/YYYY")}\``)
      await testWebhook.send({
        embeds: [test_webhook_embed]
      })
    ...
    case "vote":
      const vote_embed = new MessageEmbed()
        .setColor(colors.green)
        .setTitle("New Vote!")
        .setDescription("A new vote was recieved from Void Bots!\n\n**Vote for the bot [here](https://infinitybots.gg/bots/975450018360229908/vote)!**")
        .addFields([
          {
            name: "Voted By",
            value: `\`${client.users.cache.get(body.user).tag}\``,
            inline: true
          },
        ])
      await mainWebhook.send({
