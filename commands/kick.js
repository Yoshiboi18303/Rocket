...
module.exports = {
  ...
  execute: async (message, args) => {
    ...
    if(user.id == message.author.id) return await message.reply({ content: "You can't kick yourself!" })
    if(user.id == client.user.id) return await message.reply({ content: "You can't kick the client!" })
    if(user.id == message.guild.ownerId) return await message.reply({ content: "You can't kick the owner of this server!" })
    if(!member.kickable) return await message.reply({ content: "I can't kick that member!" })
    member.kick(reason)
      .then(async () => {
        const memberKickedEmbed = new MessageEmbed()
          .setColor(colors.green)
          .setDescription(`✅ **${user.username}** kicked for reason \`${reason}\`! ✅`)
        ...
        const kickedEmbed = new MessageEmbed()
          ...
          .setDescription(`❗ You were kicked from **${message.guild.name}** by **${message.author.tag}** (<@${message.author.id}>) for reason \`${reason}\`! ❗`)
          .setFooter({
