...
const { Interaction, Message, MessageActionRow, MessageButton, MessageEmbed } = require("discord.js");
const ms = require("ms");
...
module.exports = {
    ...
    execute: async (message, args) => {
        var user = message.mentions.users.first() || message.client.users.cache.get(args[0]) || null;
        ...
        */
        const checkIfDone = (a, b) => {
            ...
            if(!a in validResponses || !b in validResponses) throw new Error(`Argument ${!a in validResponses ? "a" : !b in validResponses ? "b" : "unknown"} has an invalid response!`)
            if(a == "rock" && b == "paper") {
                finished = true;
